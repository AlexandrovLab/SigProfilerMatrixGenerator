...
TEST_GENOMES = ['c_elegans', 'dog', 'GRCh37', 'GRCh38', 'mm9', 'mm10', 'mm39', 'rn6', 'yeast']
...
def test_genome(genome):
    matrices = matGen.SigProfilerMatrixGeneratorFunc(FILE_PREF,
                genome, os.path.join(TEST_INPUT_DIR, genome), plot=False,
...
def install_genomes(genome_install_list):
    ...
    for tmp_genome in genome_install_list:
        if tmp_genome == "all":
            ...
            print("Warning: Download for", tmp_genome, "is skipped because it is not a valid test genome.")
        else:
            genInstall.install(tmp_genome)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run tests for SigProfilerMatrixGenerator')
    # add argument --test_genome
    parser.add_argument("-t", "--test_genome",
            help="Genomes to test (GRCh37, any test genome, or all)", nargs="+",
            default=None)
    # add argument --download_genomes
    parser.add_argument("-d", "--download_genomes",
            help="Download genomes for the test (GRCh37, any test genome, or all)", nargs="+",
            default=None)
    ...
    if args.test_genome is None:
        ...
        for genome in args.test_genome:
            if genome not in TEST_GENOMES:
                print(genome + " is not a valid test genome. Please choose from: " + str(TEST_GENOMES))
                continue
            try:
                test_genome(genome)
